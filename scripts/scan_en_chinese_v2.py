import os, re, html

articles_dir = "articles"
out_path = "scripts/scan_en_chinese_results.txt"

# Pattern to find EN lang-content block (non-greedy until closing </div>)
pattern = re.compile(r'<div class="lang-content" data-lang="en">(.*?)</div>\s*</div>', re.S)
zh_char = re.compile(r'[\u4e00-\u9fff]')

results = []

for fn in sorted(os.listdir(articles_dir)):
    if not fn.endswith(".html") or fn == "index.html":
        continue
    path = os.path.join(articles_dir, fn)
    with open(path, "r", encoding="utf-8") as f:
        content = f.read()
    m = pattern.search(content)
    if not m:
        continue
    en_block = m.group(1)
    text = re.sub(r'<[^>]+>', '', en_block)
    text = html.unescape(text)
    lines = text.splitlines()
    found = []
    for i, line in enumerate(lines, 1):
        if zh_char.search(line):
            found.append((i, line.strip()))
    if found:
        results.append((fn, found))

with open(out_path, "w", encoding="utf-8") as f:
    if not results:
        f.write("No Chinese found in EN blocks.\n")
    else:
        for fn, found in results:
            f.write(f"\n=== {fn} ===\n")
            for line_no, line_text in found:
                f.write(f"  L{line_no}: {line_text}\n")

print(f"Results written to {out_path}")
print(f"Files with Chinese in EN block: {len(results)}")
for fn, _ in results:
    print(f"  - {fn}")
