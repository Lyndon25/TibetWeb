...
# -*- coding: utf-8 -*-
"""
Convert WeChat-style HTML articles to standard Tibet Moto Travel article format.
Processes files from AddingArticleWorkSpace/1/ and outputs to articles/.
"""
...
# Articles already converted (skip these)
ALREADY_CONVERTED = {
    '春染雪域桃花沐雪绘出好钱景（中英）',
    '春日经济激发西藏文旅消费市场活力（中英）',
    '绝美刷屏清明西藏出游攻略小众踏青地治愈整个春天（中英）',
    '每一张都能当壁纸这座宝藏小城的春天太惊艳了（中英）',
}
...
ARTICLES_META = [
    ...
    },
    {
        ...
        'excerptEn': 'Crossing the Changtang Plateau along G216, discover the secluded blue dreamscapes of Coqen — Renduo, Zhari Namco, Tangra Yumco.',
    },
    {
        ...
        'excerptEn': 'From Lhasa loop to Ali northern route, 6 classic Tibet itineraries spanning 3-11 days for every schedule and budget.',
    },
    {
    ...
    },
    {
        ...
        'excerptEn': 'Ali is beautiful all year round — spring flowers, summer lakes, autumn earth forests, winter blue ice. Your complete seasonal guide.',
    },
    {
        'file_pattern': '西藏旅游超全避雷攻略52条血泪总结进藏前必看',
        'slug': 'tibet-52-tips-avoid-pitfalls',
        'cat': 'health',
        'catLabel': 'Health & Safety',
        'catLabelZh': '健康与安全',
        'date': 'Feb 10, 2026',
        'time': '12 min read',
        'authorZh': '小卓玛',
        ...
        'excerptEn': '52 practical Tibet tips, from altitude prevention to shopping scams, route planning to accommodation — avoid the pitfalls.',
    },
    {
...
def extract_content_sections(soup):
    """
    Extract Chinese and English content from bilingual WeChat HTML.
    Returns (zh_elements, en_elements, cover_image).
    """
    ...
    # Find all images with data-src or src
    all_images = []
    ...
    # Try to split zh/en by looking for English paragraphs
    all_elements = []
    for elem in content_div.descendants:
        if isinstance(elem, NavigableString):
            continue
        if elem.name in ('p', 'h1', 'h2', 'h3', 'h4', 'section'):
            text = clean_text(elem.get_text())
            if text and len(text) > 2:
                all_elements.append((elem.name, text, elem))
        elif elem.name == 'img':
            src = elem.get('data-src') or elem.get('src')
            if src and src.startswith('http'):
                all_elements.append(('img', src, elem))
        elif elem.name == 'strong':
            # Check if inside p
            parent = elem.find_parent('p')
            if not parent:
                text = clean_text(elem.get_text())
                if text:
                    all_elements.append(('strong', text, elem))

    # Heuristic: split zh/en content
    # English content typically starts after a certain point or has specific patterns
    zh_elements = []
    en_elements = []

    # Look for a clear divider: if we see a long stretch of English-only paragraphs
    en_start_idx = None
    for i, (tag, text, elem) in enumerate(all_elements):
        if tag in ('p', 'h1', 'h2', 'h3', 'h4', 'strong'):
            # Check if mostly English
            ascii_chars = sum(1 for c in text if ord(c) < 128 and c.isalpha())
            total_chars = sum(1 for c in text if c.isalpha())
            if total_chars > 10 and ascii_chars / total_chars > 0.7:
                # This is English
                if en_start_idx is None:
                    # Look for a clear pattern: previous 3+ elements are Chinese
                    prev_cn = sum(1 for j in range(max(0, i-5), i)
                                  if all_elements[j][0] in ('p','h1','h2','h3','h4','strong')
                                  and sum(1 for c in all_elements[j][1] if '\u4e00' <= c <= '\u9fff') > 3)
                    if prev_cn >= 2:
                        en_start_idx = i

    if en_start_idx:
        zh_elements = all_elements[:en_start_idx]
        en_elements = all_elements[en_start_idx:]
    else:
        # Fallback: put all text in zh, minimal en
        zh_elements = all_elements
        en_elements = []

    return zh_elements, en_elements, cover_image


def render_elements(elements):
    """Convert extracted elements to HTML string."""
    html_parts = []
    for tag, text, elem in elements:
        if tag == 'img':
            html_parts.append(f'<img src="{text}" alt="" loading="lazy">')
        elif tag in ('h1', 'h2', 'h3', 'h4'):
            # Map h1/h2 to h2, h3/h4 to h3 for article body
            if tag in ('h1', 'h2'):
...
def build_article_html(meta, zh_html, en_html, prev_slug=None, next_slug=None):
    """Build the standard article HTML from template."""
    prev_link = ''
    if prev_slug:
        # Need to find title for prev
    ...
    # Default related articles (4 from existing pool)
    related = [
    ...
    # Determine cover image from content or fallback
    cover = meta.get('cover', '')
    if not cover:
        # Try to extract first image from zh_html
        m = re.search(r'<img[^>]+src="([^"]+)"', zh_html)
        if m:
            cover = m.group(1)
...
def process_file(filepath, meta, prev_slug=None, next_slug=None):
    """Process a single WeChat HTML file."""
    print(f"Processing: {os.path.basename(filepath)} -> {meta['slug']}.html")

    with open(filepath, 'r', encoding='utf-8') as f:
        html = f.read()

    soup = BeautifulSoup(html, 'html.parser')
    zh_elements, en_elements, cover_image = extract_content_sections(soup)

    meta['cover'] = cover_image

    zh_html = render_elements(zh_elements)
    en_html = render_elements(en_elements)

    # If no English content found, generate minimal placeholder
    if not en_html.strip():
        en_html = f'<p>{meta["excerptEn"]}</p>'

    article_html = build_article_html(meta, zh_html, en_html, prev_slug, next_slug)

    out_path = os.path.join('articles', f"{meta['slug']}.html")
    with open(out_path, 'w', encoding='utf-8') as f:
        f.write(article_html)

    print(f"  Written: {out_path}  ({len(zh_html)} zh chars, {len(en_html)} en chars)")
    return True
...
def main():
    ...
    # Map file patterns to actual files
    ...
    for meta in ARTICLES_META:
        ...
        if matched:
            ...
            print(f"WARNING: No file found for pattern: {meta['file_pattern']}")

    # Sort by date for prev/next chain
    ...
    for i, (filepath, meta) in enumerate(to_process):
        ...
        process_file(filepath, meta, prev_slug, next_slug)
